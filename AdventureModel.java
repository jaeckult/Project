...
public class AdventureModel {
	...
	public void initGame(){
		...
		//defining objects present in the game
		things[0] = new Thing("Gold", "A gold is hidden in this room");
	    things[1] = new Thing("Emperor", "An Emperor is taking care of room");
	    ...
	    things[6] = new Thing("Spear", "A Sword to shoot Enemy");

	    //defining rooms and objects they contain
	    rooms[0] = new Room("Aksum", "Aksum contains 2 Golds and pick up items command");
	    rooms[0].addObject(things[0]);
	    rooms[0].addObject(things[0]);

	    rooms[1] = new Room("Lalibela", "Lalibela needs items");
	    rooms[2] = new Room("Gondar", "Gondar pick up items");
	    rooms[3] = new Room("Addis Ababa", "Addis Ababa pick up items and start position");

	    rooms[4] = new Room("Harar", "Harar contains pick up items and Army");
	    rooms[4].addObject(things[4]);

	    rooms[5] = new Room("Bahir Dar", "Bahir Dar contains pick up items and Emperor");
	    rooms[5].addObject(things[1]);

	    rooms[6] = new Room("Dire Dawa", "Dire Dawa contains pick up items, Reqruired command,Gold and Boat");
	    rooms[6].addObject(things[0]);
	    rooms[6].addObject(things[5]);

	    rooms[7] = new Room("Hawassa", "Hawassa contains pick up items, Reqruired command,Emperor and Banner");
	    rooms[7].addObject(things[1]);
	    rooms[7].addObject(things[2]);

	    rooms[8] = new Room("Mekelle", "Mekelle contains pick up items, Reqruired command,Food and Banner");
	    rooms[8].addObject(things[3]);
	    rooms[8].addObject(things[2]);

	    rooms[9] = new Room("Dessie", "Dessie contains pick up items, Reqruired command,Spear and 2 Banner");
	    rooms[9].addObject(things[6]);
	    rooms[9].addObject(things[2]);
	    rooms[9].addObject(things[2]);

	    rooms[10] = new Room("Mojo", "Mojo contains pick up items and Food");
	    rooms[10].addObject(things[3]);

	    rooms[11] = new Room("Shire", "Shire contains pick up items,sticker and Food");
	    rooms[11].addObject(things[2]);
	    rooms[11].addObject(things[3]);

	    rooms[12] = new Room("Arbaminch", "Arbaminch contains pick up items,sticker and Food");
	    rooms[12].addObject(things[0]);
	    rooms[12].addObject(things[2]);

	    rooms[13] = new Room("Jimma", "Jimma contains pick up items,sticker and Food");
	    rooms[13].addObject(things[2]);


	    rooms[14] = new Room("Adama", "Adama contains pick up items,Items required,sticker and Food");
	    rooms[14].addObject(things[2]);
	    rooms[14].addObject(things[3]);

	    rooms[15] = new Room("Addis Ababa(End)", "Addis Ababa(End) and items required Emperor,Sticker, and Army");
	    rooms[15].addObject(things[1]);
	    rooms[15].addObject(things[2]);
	    rooms[15].addObject(things[4]);


		//mapping wall direction
	...
	public void startGame(){
		...
		do{
			...
			if(cmd.equalsIgnoreCase("Go")){
				do {
					System.out.println(currentLoc + "\nChoose direction: one of north, south, east, west");
					...
					if(b) {
						...
						System.out.println("Here is Water or Wall" + "/nYou can't Move to this direction");
					}
				}while(b==false);       
			}
